...
4. JWT **RS256** (issuer `painel-robo`) com `jti`, `permissions` e `user.email`; entregue no cookie `robo_auth_token` **host-only** (sem `Domain=.juri.capital`). O `auth_token` (`.juri.capital`) é só lido (cookie da juri-api).
...
- A sessão própria usa o cookie `robo_auth_token` **host-only** (nome distinto do `auth_token` para não colidir no cookie-parser); a API só **lê** o `auth_token` que a juri-api seta em `.juri.capital`. Em `NODE_ENV=local`, sem `Secure`. As opções de set e clear precisam casar, senão o cookie fica órfão.
